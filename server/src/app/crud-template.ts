...
export abstract class crudtemplate<T extends {id: string}> {
    ...
    create(socket: Socket) {
        socket.on(`create-${this.name}`, (data: T) => { // create Room
        this.makeItem(data)
    ...
    read(socket: Socket) {
        socket.on(`get-all-${this.name}`, () => { // Read Rooms
            this.emitUpdate();
        })
    }
    
    update(socket: Socket) {
        socket.on(`update-${this.name}`, (id : string) => { // Update Room
            this.emitUpdate({socket, id});
        })
    }
    
    delete(socket: Socket) {
        socket.on(`delete-${this.name}`, (id: string) => { // Delete Room
